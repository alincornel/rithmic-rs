...
impl PnlPlant {
    async fn send_or_fail(&mut self, msg: Message, request_id: &str) {
        match tokio::time::timeout(Duration::from_secs(10), self.rithmic_sender.send(msg)).await {
            Ok(Ok(_)) => {}
            Ok(Err(e)) => {
                error!("pnl_plant: WebSocket send failed for request {}: {e}", request_id);
                self.request_handler.fail_request(request_id, RithmicError::SendFailed);
            }
            Err(_) => {
                error!("pnl_plant: WebSocket send TIMED OUT (10s) for request {}", request_id);
                self.request_handler.fail_request(request_id, RithmicError::SendFailed);
            }
        }
    }
            ...
            tokio::select! {
                ...
                _ = self.ping_interval.tick() => {
                    ...
                    match tokio::time::timeout(
                        ...
                        Ok(Ok(_)) => {}
                        Ok(Err(e)) => {
                            error!("pnl_plant: ping send failed: {e} — connection dead");
                        ...
                        Err(_) => {
                            error!("pnl_plant: ping send TIMED OUT (10s) — forcing reconnect");
                            let error_response = RithmicResponse {
